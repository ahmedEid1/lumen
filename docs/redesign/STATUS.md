...
| 9 | `<RadioGroup>` + `<Checkbox>` primitives + quiz radiogroup a11y migration | Shipped. 37 files / 202 tests green (+1 file / +8 tests). Closes AUDIT.md §3 Block-renderer's heaviest a11y finding — quiz options now have role/aria-checked, arrow-key nav, fieldset/legend. | _pending_ |
