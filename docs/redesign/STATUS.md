...
| 14 | Foundation E (full tier) — Tabs + Breadcrumb + DataTable + 5 migrations + ScoreBadge token-drift fix | Shipped (BIG loop, ~1500 LoC per user feedback "team day of work"). 46 files / 260 tests green (+3 files / +16 tests). Three primitives ship together: Tabs (Radix), Breadcrumb (custom), DataTable (custom, minimum-viable API: columns + rows + sort intent + empty/loading slots). Migrations: /studio status filter + /admin/observability tab rail → Tabs; /admin/users + /admin/courses + /admin/audit → DataTable; /studio/[id] gets a Breadcrumb. ScoreBadge + eval-report borders swap `text-emerald/amber/rose` literals for semantic `text-success/warning/destructive`. **Closes Foundation E** — every audit-flagged primitive in §2 is now in the kit. | _pending_ |
