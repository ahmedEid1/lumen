...
| 19 | OG image + AUDIT §7-19 cleanup | New `app/opengraph-image.tsx` (Next 15 file convention) — was a layout `openGraph` metadata blob with no image, so social shares rendered text-only. Workbench-styled 1200×630 card: black bg, lime brand mark, "Now open." cartouche, display-face hero. Tests still 50/284 green. Portfolio screenshot regen + Lighthouse formal score deferred to FINAL-REPORT — depend on infra (agentic_demo seed + Chrome CDP) outside the redesign-loop scope. | _pending_ |
