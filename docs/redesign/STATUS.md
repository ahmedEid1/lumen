...
| 15 | Auth polish — PasswordInput + StrengthMeter + register confirm/T&C + verify idempotency + LinkButton sweep + Codex rescue #4 | Shipped under new LOCAL-FIRST workflow: lint + tsc + 48 files/275 tests + dev-browser walk + local axe (12 passed / 1 flaky) + Codex rescue all verified before push. 2 P1 findings caught in-loop (untracked primitives = diff-scope artifact; auth.spec.ts register flow broken by new gating — fixed). All 8 e2e password-label selectors swept to `getByLabel("Password", { exact: true })`. Closes AUDIT §3 Auth surfaces backlog. | `3716077` |
| 16 | Block renderer + course detail polish (BIG) | Shiki syntax highlighting (dynamic-imported), image aspect-ratio reservation, lesson video with poster/buffering/error fallback, past-attempt pills lucide-ified, quiz short-answer → Input. Course detail decomposed 444-LoC monolith into 5 components (CourseHeader / Outcomes / Syllabus / Reviews / Sidebar). Shape-matching Skeleton, error+recovery branch, router.push enroll, PDF cert 401-fallback. Callout token-drift fix. 48 files/275 tests green. | _pending_ |
