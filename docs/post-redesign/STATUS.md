...
| 21a | Streaming spine (flag OFF) — 4 new endpoints (POST /tutor/turns, status, SSE stream, cancel) all gated 503 until L21b; Celery `tutor.run_turn.v1` task with atomic phase fence + `after_commit` enqueue + finally-suppress cleanup; sweep beat (10s pending / 30s running) + orphan-stream cleanup beat (5min); Redis Streams helpers with trim detection; AsyncIterator orchestrator (noop event sequence). +7 backend tests. Suite 701 → 708 green. | TBD |
