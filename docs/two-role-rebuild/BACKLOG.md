...
- **P2 — streamed tutor turns don't persist messages on prod.** Found 2026-06-06 during the README capture session (demo@lumen.test, `rag-from-scratch`): a streamed turn answers correctly in the panel (retriever fires, tokens stream, turn completes) but `GET /api/v1/tutor/conversations/{id}` returns `messages: []` even 120 s later, and the tutor history is empty after a page reload — the conversation row exists, the user+assistant messages never land. Two separate turns reproduced it. Means: learners lose their tutor history on refresh, and the per-turn trace drill-down (`/dashboard/tutor/{conv}/turn/{msg}`) is unreachable for streamed turns (no message id ever exists). The non-streaming path persists fine (curated seed turns render). Suspect the worker-side persistence step in `app/workers/tasks/tutor_streaming.py` (or its claim/fence path) soft-fails on prod — check worker logs around `turn_complete`.
