...
- **Production-grade observability** *(shipped — Phase H, items H1 + H7)* — every LLM call's prompt/completion tokens, USD cost, latency, and outcome land in the `llm_calls` table (Alembic 0022) via [`apps/backend/app/services/llm_call_log.py`](apps/backend/app/services/llm_call_log.py). The per-user 24h budget guard returns HTTP 429 `llm.budget_exceeded` once the threshold trips. `/admin/observability` adds Celery queue depth, retrieval-quality drill-down, and a per-trace expander; learners get their own trace surface at `/me/traces` powered by [`learner_traces.py`](apps/backend/app/services/learner_traces.py) + [`agent_tracer.py`](apps/backend/app/services/agent_tracer.py) (I4).
