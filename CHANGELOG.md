...
- **Scrubbed Meilisearch references from CLAUDE.md + `app/cli.py`** — Meilisearch was retired from `docker-compose.yml` and `app.core.config.Settings` earlier in the rebuild but CLAUDE.md still listed it under "Data:" and `app/cli.py info` accessed a non-existent `s.search_backend` (latent `AttributeError`). The search index now lives in Postgres `tsvector` (`apps/backend/app/repositories/courses.py`); the Celery reindex task continues to be the trigger.
