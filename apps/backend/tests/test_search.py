...
async def _publish(client: AsyncClient, headers: dict, subject_id: str, title: str, overview: str) -> str:
...
async def test_search_finds_by_title(client: AsyncClient, auth_headers, db_session) -> None:
    teacher = await auth_headers(role=Role.instructor)
    subject = await _make_subject(db_session)
    await _publish(client, teacher, subject.id, "Async Python deep dive", "Coroutines.")
    await _publish(client, teacher, subject.id, "JavaScript essentials", "Closures.")
...
async def test_search_filters_difficulty(client: AsyncClient, auth_headers, db_session) -> None:
    teacher = await auth_headers(role=Role.instructor)
    subject = await _make_subject(db_session)
    await _publish(client, teacher, subject.id, "Easy course", "intro stuff")
