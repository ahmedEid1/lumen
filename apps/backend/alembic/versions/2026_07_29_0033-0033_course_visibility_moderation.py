...
revision: str = "0033"
# INTEGRATION: re-pointed at the S2 merge. This worktree branched from the
# foundation rev 0030 (S7-pre) before S1 (0031 role data-collapse, 0032 default
# flip) and S5 (0038 BYOK credentials, 0039 llm_calls billing_mode, 0040 tutor
# turn credential_id) had merged. Those streams landed first, so the post-S5
# head is 0040; the consolidated linear chain puts this S2 visibility migration
# directly after it. ``down_revision`` was rebased 0030 -> 0040 at merge so the
# chain stays linear with no dangling/duplicate down_revisions (verified via
# ``alembic heads`` -> single head 0044 and ``test_migration_chain``, S7.10).
